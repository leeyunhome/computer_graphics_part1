# 인터랙티브 데모

WebGPU Compute Shader로 DirectX 11 강의 예제를 재현한 데모입니다.  

슬라이더를 조작해 실시간으로 파라미터를 바꿔볼 수 있습니다.


---


## [1차원 배열을 통한 2D 캔버스 제어와 CPU-GPU 메모리 매핑](1차원-배열을-통한-2d-캔버스-제어와-cpu-gpu-메모리-매핑.md)


[→ 데모 열기](1차원-배열을-통한-2d-캔버스-제어와-cpu-gpu-메모리-매핑.md){.md-button}


---


## [1차원 배열을 통한 2D 캔버스 제어와 프레임 기반 픽셀 애니메이션](1차원-배열을-통한-2d-캔버스-제어와-프레임-기반-픽셀-애니메이션.md)


[→ 데모 열기](1차원-배열을-통한-2d-캔버스-제어와-프레임-기반-픽셀-애니메이션.md){.md-button}


---


## [1차원 버퍼와 프레임 업데이트를 통한 픽셀 애니메이션 구현](1차원-버퍼와-프레임-업데이트를-통한-픽셀-애니메이션-구현.md)


[→ 데모 열기](1차원-버퍼와-프레임-업데이트를-통한-픽셀-애니메이션-구현.md){.md-button}


---


## [Bloom 효과 파이프라인 설계 및 2차원 공간 필터링 구조화](bloom-효과-파이프라인-설계-및-2차원-공간-필터링-구조화.md)


[→ 데모 열기](bloom-효과-파이프라인-설계-및-2차원-공간-필터링-구조화.md){.md-button}


---


## [Box Blur 5x5: 수평 패스 구현 및 분리형 컨볼루션 이해](box-blur-5x5-수평-패스-구현-및-분리형-컨볼루션-이해.md)


[→ 데모 열기](box-blur-5x5-수평-패스-구현-및-분리형-컨볼루션-이해.md){.md-button}


---


## [boxblur5 분리 가능한 컨볼루션수평 패스 구현 리뷰](boxblur5-분리-가능한-컨볼루션수평-패스-구현-리뷰.md)


[→ 데모 열기](boxblur5-분리-가능한-컨볼루션수평-패스-구현-리뷰.md){.md-button}


---


## [[DirectX 11] 3D 공간 변환과 렌더링 파이프라인의 핵심 구현](directx-11-3d-공간-변환과-렌더링-파이프라인의-핵심-구현.md)


[→ 데모 열기](directx-11-3d-공간-변환과-렌더링-파이프라인의-핵심-구현.md){.md-button}


---


## [DirectX 11 / C++ 그래픽스 스터디: 이미지 교체 및 렌더 타겟 초기화 커밋 분석](directx-11-c-그래픽스-스터디-이미지-교체-및-렌더-타겟-초기화.md)


[→ 데모 열기](directx-11-c-그래픽스-스터디-이미지-교체-및-렌더-타겟-초기화.md){.md-button}


---


## [[DirectX 11] C++ 그래픽스 환경에서의 GLM 실험적 확장 활성화 및 디버깅 기초](directx-11-c-그래픽스-환경에서의-glm-실험적-확장-활성화-및.md)


[→ 데모 열기](directx-11-c-그래픽스-환경에서의-glm-실험적-확장-활성화-및.md){.md-button}


---


## [DirectX 11 그래픽스 파이프라인의 첫 걸음: Window 및 디바이스 초기화](directx-11-그래픽스-파이프라인의-첫-걸음-window-및-디바이.md)


[→ 데모 열기](directx-11-그래픽스-파이프라인의-첫-걸음-window-및-디바이.md){.md-button}


---


## [DirectX 11 포스트 프로세싱: 블룸(Bloom) 필터 구현과 렌더 타겟 초기화 분석](directx-11-포스트-프로세싱-블룸bloom-필터-구현과-렌더-타겟.md)


[→ 데모 열기](directx-11-포스트-프로세싱-블룸bloom-필터-구현과-렌더-타겟.md){.md-button}


---


## [DirectX 11/C++ 그래픽스 스터디: 첫 커밋 분석](directx-11c-그래픽스-스터디-첫-커밋-분석.md)


[→ 데모 열기](directx-11c-그래픽스-스터디-첫-커밋-분석.md){.md-button}


---


## [GLM 실험적 확장 활성화를 통한 C++ 그래픽스 디버깅 환경 구축](glm-실험적-확장-활성화를-통한-c-그래픽스-디버깅-환경-구축.md)


[→ 데모 열기](glm-실험적-확장-활성화를-통한-c-그래픽스-디버깅-환경-구축.md){.md-button}


---


## [GLM을 활용한 3차원 벡터 연산의 기초: 내적, 외적, 그리고 DirectXMath와의 비교](glm을-활용한-3차원-벡터-연산의-기초-내적-외적-그리고-directx.md)


[→ 데모 열기](glm을-활용한-3차원-벡터-연산의-기초-내적-외적-그리고-directx.md){.md-button}


---


## [GLM을 활용한 벡터 연산의 수학적 이해 및 DirectXMath와의 비교 분석](glm을-활용한-벡터-연산의-수학적-이해-및-directxmath와의-비.md)


[→ 데모 열기](glm을-활용한-벡터-연산의-수학적-이해-및-directxmath와의-비.md){.md-button}


---


## [ImGui를 활용한 캔버스 배경색 제어 구현](imgui를-활용한-캔버스-배경색-제어-구현.md)


[→ 데모 열기](imgui를-활용한-캔버스-배경색-제어-구현.md){.md-button}


---


## [ImGui와 DirectX 11을 이용한 실시간 캔버스 컬러 동적 제어](imgui와-directx-11을-이용한-실시간-캔버스-컬러-동적-제어.md)


[→ 데모 열기](imgui와-directx-11을-이용한-실시간-캔버스-컬러-동적-제어.md){.md-button}


---


## [픽셀 버퍼 애니메이션](pixel-animation.md)


[→ 데모 열기](pixel-animation.md){.md-button}


---


## [CPU 레이트레이서 — Phong 셰이딩 + 그림자](raytracer.md)


[→ 데모 열기](raytracer.md){.md-button}


---


## [광선-구체 교차 판정과 법선 벡터 계산의 수학적 구현](광선-구체-교차-판정과-법선-벡터-계산의-수학적-구현.md)


[→ 데모 열기](광선-구체-교차-판정과-법선-벡터-계산의-수학적-구현.md){.md-button}


---


## [광선-삼각형 교점 계산의 첫 걸음: 광선-평면 교차 수식 유도 및 구현](광선-삼각형-교점-계산의-첫-걸음-광선-평면-교차-수식-유도-및-구현.md)


[→ 데모 열기](광선-삼각형-교점-계산의-첫-걸음-광선-평면-교차-수식-유도-및-구현.md){.md-button}


---


## [[리뷰] Phong Shading의 핵심, 스펙큘러(Specular) 반사 벡터 구현 및 조명 모델 완성](리뷰-phong-shading의-핵심-스펙큘러specular-반사-벡터-.md)


[→ 데모 열기](리뷰-phong-shading의-핵심-스펙큘러specular-반사-벡터-.md){.md-button}


---


## [[리뷰] 레이트레이싱 기반 퐁 반사 모델(Phong Reflection Model)의 기초 구현](리뷰-레이트레이싱-기반-퐁-반사-모델phong-reflection-mod.md)


[→ 데모 열기](리뷰-레이트레이싱-기반-퐁-반사-모델phong-reflection-mod.md){.md-button}


---


## [[리뷰 및 학습 가이드] Separable 5-Tap Box Blur 구현을 통한 2D 컨볼루션 최적화](리뷰-및-학습-가이드-separable-5-tap-box-blur-구현을.md)


[→ 데모 열기](리뷰-및-학습-가이드-separable-5-tap-box-blur-구현을.md){.md-button}


---


## [[리뷰] 블룸(Bloom) 효과의 첫 단계: 휘도(Luminance) 기반 임계값 추출 구현](리뷰-블룸bloom-효과의-첫-단계-휘도luminance-기반-임계값-추.md)


[→ 데모 열기](리뷰-블룸bloom-효과의-첫-단계-휘도luminance-기반-임계값-추.md){.md-button}


---


## [벡터 외적을 이용한 삼각형의 법선 벡터 및 면적 계산](벡터-외적을-이용한-삼각형의-법선-벡터-및-면적-계산.md)


[→ 데모 열기](벡터-외적을-이용한-삼각형의-법선-벡터-및-면적-계산.md){.md-button}


---


## [분리형 가우시안 블러(Separable Gaussian Blur)를 이용한 Bloom 효과 구현 리뷰](분리형-가우시안-블러separable-gaussian-blur를-이용한-.md)


[→ 데모 열기](분리형-가우시안-블러separable-gaussian-blur를-이용한-.md){.md-button}


---


## [삼각형의 평면성(Planarity)과 비평면 사각형의 삼각형 분할(Triangulation)](삼각형의-평면성planarity과-비평면-사각형의-삼각형-분할triang.md)


[→ 데모 열기](삼각형의-평면성planarity과-비평면-사각형의-삼각형-분할triang.md){.md-button}


---


## [선형 톤 매핑과 오버플로우 방지를 위한 픽셀 클램핑(Clamping)](선형-톤-매핑과-오버플로우-방지를-위한-픽셀-클램핑clamping.md)


[→ 데모 열기](선형-톤-매핑과-오버플로우-방지를-위한-픽셀-클램핑clamping.md){.md-button}


---


## [선형 톤 매핑과 클램핑을 통한 이미지 밝기 조절 및 오버플로우 방지](선형-톤-매핑과-클램핑을-통한-이미지-밝기-조절-및-오버플로우-방지.md)


[→ 데모 열기](선형-톤-매핑과-클램핑을-통한-이미지-밝기-조절-및-오버플로우-방지.md){.md-button}


---


## [스크린 공간에서의 원 판별 알고리즘 구현 (Point-in-Circle Test)](스크린-공간에서의-원-판별-알고리즘-구현-point-in-circle-t.md)


[→ 데모 열기](스크린-공간에서의-원-판별-알고리즘-구현-point-in-circle-t.md){.md-button}


---


## [스크린 공간에서의 원 판정(Point-in-Circle) 알고리즘 구현 및 최적화](스크린-공간에서의-원-판정point-in-circle-알고리즘-구현-및-.md)


[→ 데모 열기](스크린-공간에서의-원-판정point-in-circle-알고리즘-구현-및-.md){.md-button}


---


## [실시간 픽셀 감쇄(Pixel Decay)를 통한 점진적 암전 효과 구현](실시간-픽셀-감쇄pixel-decay를-통한-점진적-암전-효과-구현.md)


[→ 데모 열기](실시간-픽셀-감쇄pixel-decay를-통한-점진적-암전-효과-구현.md){.md-button}


---


## [원근 투영(Perspective Projection)과 최단 거리 충돌 판정(Closest Collision)의 구현](원근-투영perspective-projection과-최단-거리-충돌-판정.md)


[→ 데모 열기](원근-투영perspective-projection과-최단-거리-충돌-판정.md){.md-button}


---


## [원근 투영과 최접점 가려짐 처리를 통한 3차원 레이트레이싱 구현](원근-투영과-최접점-가려짐-처리를-통한-3차원-레이트레이싱-구현.md)


[→ 데모 열기](원근-투영과-최접점-가려짐-처리를-통한-3차원-레이트레이싱-구현.md){.md-button}


---


## [이미지 밝기 조절 로직 추가 픽셀 단위 연산의 시작](이미지-밝기-조절-로직-추가-픽셀-단위-연산의-시작.md)


[→ 데모 열기](이미지-밝기-조절-로직-추가-픽셀-단위-연산의-시작.md){.md-button}


---


## [이미지 밝기 조절 픽셀 값 직접 조작하기](이미지-밝기-조절-픽셀-값-직접-조작하기.md)


[→ 데모 열기](이미지-밝기-조절-픽셀-값-직접-조작하기.md){.md-button}


---


## [[코드 리뷰] CPU 픽셀 버퍼 제어와 RGB 순환 애니메이션 구현](코드-리뷰-cpu-픽셀-버퍼-제어와-rgb-순환-애니메이션-구현.md)


[→ 데모 열기](코드-리뷰-cpu-픽셀-버퍼-제어와-rgb-순환-애니메이션-구현.md){.md-button}


---


## [[코드 리뷰] Separable 5-Tap Box Blur 가로 패스 구현 및 이미지 전처리](코드-리뷰-separable-5-tap-box-blur-가로-패스-구현-.md)


[→ 데모 열기](코드-리뷰-separable-5-tap-box-blur-가로-패스-구현-.md){.md-button}


---


## [[코드 리뷰] Separable 5-Tap Gaussian Blur 구현 분석 및 최적화](코드-리뷰-separable-5-tap-gaussian-blur-구현-분.md)


[→ 데모 열기](코드-리뷰-separable-5-tap-gaussian-blur-구현-분.md){.md-button}


---


## [[코드 리뷰] Separable Convolution을 활용한 Box Blur 5 가로 패스 구현](코드-리뷰-separable-convolution을-활용한-box-blu.md)


[→ 데모 열기](코드-리뷰-separable-convolution을-활용한-box-blu.md){.md-button}


---


## [[코드 리뷰] 스크린-월드 좌표계 변환 및 포스트 프로세싱(Bloom) 합성 구현](코드-리뷰-스크린-월드-좌표계-변환-및-포스트-프로세싱bloom-합성-구.md)


[→ 데모 열기](코드-리뷰-스크린-월드-좌표계-변환-및-포스트-프로세싱bloom-합성-구.md){.md-button}


---


## [[코드 리뷰] 실시간 템포럴 라이트 디케이(Temporal Light Decay)와 이미지 프로세싱 파이프라인 구현](코드-리뷰-실시간-템포럴-라이트-디케이temporal-light-deca.md)


[→ 데모 열기](코드-리뷰-실시간-템포럴-라이트-디케이temporal-light-deca.md){.md-button}


---


## [[코드 리뷰] 이미지 밝기(Brightness) 조절의 기초와 픽셀 연산](코드-리뷰-이미지-밝기brightness-조절의-기초와-픽셀-연산.md)


[→ 데모 열기](코드-리뷰-이미지-밝기brightness-조절의-기초와-픽셀-연산.md){.md-button}


---


## [퐁 반사 모델(Phong Reflection Model)의 스펙큘러 하이라이트 및 반사 벡터 구현](퐁-반사-모델phong-reflection-model의-스펙큘러-하이라이.md)


[→ 데모 열기](퐁-반사-모델phong-reflection-model의-스펙큘러-하이라이.md){.md-button}


---


## [[피드백] Separable 5-tap Gaussian Blur 구현 분석 및 최적화 제안](피드백-separable-5-tap-gaussian-blur-구현-분석-.md)


[→ 데모 열기](피드백-separable-5-tap-gaussian-blur-구현-분석-.md){.md-button}


---


## [픽셀 버퍼 애니메이션 이해와 DirectX 11 적용](픽셀-버퍼-애니메이션-이해와-directx-11-적용.md)


[→ 데모 열기](픽셀-버퍼-애니메이션-이해와-directx-11-적용.md){.md-button}


---


## [픽셀 애니메이션 구현과 프레임 기반 갱신 원리](픽셀-애니메이션-구현과-프레임-기반-갱신-원리.md)


[→ 데모 열기](픽셀-애니메이션-구현과-프레임-기반-갱신-원리.md){.md-button}


---


## [픽셀 애니메이션 및 프레임 제어 구현](픽셀-애니메이션-및-프레임-제어-구현.md)


[→ 데모 열기](픽셀-애니메이션-및-프레임-제어-구현.md){.md-button}


---


## [픽셀 조작을 통한 캔버스 초기화 이해: (2,0) 위치에 파란색 픽셀 추가](픽셀-조작을-통한-캔버스-초기화-이해-20-위치에-파란색-픽셀-추가.md)


[→ 데모 열기](픽셀-조작을-통한-캔버스-초기화-이해-20-위치에-파란색-픽셀-추가.md){.md-button}


---


## [학습자 커밋 분석: add source code](학습자-커밋-분석-add-source-code.md)


[→ 데모 열기](학습자-커밋-분석-add-source-code.md){.md-button}


---


## [학습자의 directx 11 c 그래픽스 스터디 커밋 분석 입력 이미지](학습자의-directx-11-c-그래픽스-스터디-커밋-분석-입력-이미지-.md)


[→ 데모 열기](학습자의-directx-11-c-그래픽스-스터디-커밋-분석-입력-이미지-.md){.md-button}


---
