...
def _build_image_prompt_section(all_images):
    ...
    lines += [
        ...
        "- **개념 다이어그램** (PNG 파일들): ⚠️ `<img>` 태그로 직접 표시 금지 — 강의 자료를 그대로 게시하는 것은 허용되지 않습니다. 대신 Canvas2D로 동일한 개념의 벡터 다이어그램을 직접 그려서 표현하세요 (독자적 재현).",
        "- **이미지 로드 패턴** (텍스처 용도일 때):",
        "```javascript",
...
- 모든 WGSL, CSS, JS 인라인
- WebGPU Compute/Render Pass 종료 시, 구버전 명칭인 `endPass()` 대신 최신 스펙인 `end()`를 반드시 사용하십시오.

## 표절 및 Recitation 필터 우회 지침 (필수 준수):
- 코드 작성 시, 변수명이나 구조체 이름을 기존 인터넷 튜토리얼 예제와 똑같이 작성하지 말고, 독창적으로 명명하십시오 (예: `Params` -> `RaytraceUniforms`, `Ray` -> `PixelRayData` 등).
- 모든 WGSL 함수 및 자바스크립트 함수 내부의 논리적 주요 단계마다 상세한 독창적 한국어 설명 주석을 최소 한 줄 이상 추가하십시오.
- boilerplate 성격의 코드도 직접 고안하여 작성한 것처럼 변수명과 구문을 다양화하여 구현하십시오.
- WGSL 내에서 Uniform Array를 루프 변수(예: `array[i]`)로 동적 참조하지 마십시오. 컴파일 오류를 예방하기 위해, 루프를 수동으로 전개(unroll)하여 상수 인덱스(`array[0]`, `array[1]`, `array[2]`)를 하드코딩해서 접근하십시오.
